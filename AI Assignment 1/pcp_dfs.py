def dfs_successor(top_fringe,top_node,bottom_fringe,bottom_node,top,bottom):
    for i in range (0,len(top)): 
        top_fringe.insert(0,top_node+ top[i])
        bottom_fringe.insert(0,bottom_node+ bottom[i]) 
    
def goal(top_node,bottom_node):
...
def pcp_bfs(lst):
    ...
    while loop:
        if top_fringe==[] or bottom_fringe==[]:
            ...
            if goal(top_node,bottom_node):
                print( "Solution is: ", top_node)
                loop= False
            else:
                dfs_successor(top_fringe,top_node,bottom_fringe,bottom_node,top,bottom)    
